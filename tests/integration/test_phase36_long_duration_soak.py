#!/usr/bin/env python
"""
Phase 36 — Integration Tests for Long-Duration Soak.
...
from scripts.phase36_long_duration_soak import SoakTestRunner


@pytest.mark.integration
@pytest.mark.live
class TestPhase36Integration:
    """Integration tests for Phase 36 soak test with real streams."""
    ...
    def test_short_soak_run(self, runner):
        ...
        try:
            ...
            assert "frame_continuity" in cam1
            assert "timestamp_monotonicity" in cam1
            assert "camera_id_integrity" in cam1
            assert "health_state" in cam1
            assert "queue_buffer" in cam1
            assert "inference_latency" in cam1
            assert "processing_fps" in cam1
            assert "source_fps" in cam1
...
class TestPhase36Regression:
    """Regression tests for Phase 36 - verify Phase 32-35 contracts still work."""
